...
uint8_t PMAESSBox(uint8_t index) {
    return PMAESSBoxData[index];
}

uint8_t PMAESSBoxInvData[256] =
{
...
uint8_t PMAESSBoxInv(uint8_t index) {
    return PMAESSBoxInvData[index];
}

uint8_t PMAESRconData[256] = {
...
uint8_t PMAESRcon(uint8_t index) {
    return PMAESRconData[index];
}
...
PMAESRoundKey PMAESGenRoundKey(uint8_t round, PMAESKey key, PMAESBlock pt) {
    PMAESWord PMKeySequence[60];
    
    for (int k = 0; k < 8; k++) {
        for (int l = 0; l < 4; l++) {
            PMKeySequence[k].data[l] = key.data[(k * 4) + l];
        }
    }
